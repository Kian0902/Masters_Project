...
from KIAN_Net_model import FuDMLP, KIANNet, he_initialization
...
Pairs = Matching3Pairs(ionogram_folder, radar_folder, geophys_folder)
rad, ion, geo = Pairs.find_pairs()
print("Pairs Matched!")
...
kian_net = KIANNet(iono_cnn.conv, geo_dmlp.fc1, fu_dmlp).to(device)
...
optimizer = optim.Adam([p for p in kian_net.parameters() if p.requires_grad], lr=0.001)
...
best_model_weights = kian_net.state_dict()


train_losses = []
val_losses = []
for epoch in range(num_epochs):
    kian_net.train()  # Set model to training mode
    running_loss = 0.0
    for i, (images, geophys, targets) in enumerate(train_loader):
        ...
        outputs = kian_net(images, geophys)
    ...
    # Validation loop
    kian_net.eval()  # Set model to evaluation mode
    val_loss = 0.0
    val_outputs = []
    val_targets = []
    with torch.no_grad():
...
torch.save(best_model_weights, 'KIAN_Net_v1_nograd.pth')
