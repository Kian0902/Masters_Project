...
from storing_dataset import MatchingGeoPair, StoreGeoDataset


from Geo_DMLP_utils import from_strings_to_array, filter_artist_times, load_dict, save_dict, convert_pred_to_dict, from_array_to_datetime, from_strings_to_datetime, from_csv_to_numpy, add_key_from_dict_to_dict, add_key_with_matching_times, inspect_dict, convert_geophys_to_dict
...
if __name__ == "__main__":
    ...
    A = StoreGeoDataset(geo)
    
    # Path to trained weights
    weights_path = 'Geo_DMLPv1.pth'
...
#     # Plotting DL model
#     ax1.pcolormesh(r_time, r_h, ne_pred, shading='auto', cmap='turbo', norm=colors.LogNorm(vmin=1e10, vmax=1e12))
#     ax1.set_title('Geo-DMLP', fontsize=17)
#     ax1.set_xlabel('Time [hh:mm]', fontsize=13)
#     ax1.xaxis.set_major_formatter(DateFormatter('%H:%M'))
#     ax1.tick_params(labelleft=False)
...
#     # Initializing class for matching pairs
#     Pairs = MatchingPairs(test_geophys_folder, test_radar_folder)
    
    
#     # Returning matching sample pairs
#     rad, geo, radar_times = Pairs.find_pairs(return_date=True)
#     r_t = from_strings_to_datetime(radar_times)
#     r_times = from_strings_to_array(radar_times)
...
#     # Adding 'r_h' from eiscat to all dicts
#     Eiscat_support = load_dict("X_test_flow")
#     X_eis = add_key_from_dict_to_dict(Eiscat_support, X_eis, key="r_h")
#     X_eis = add_key_with_matching_times(Eiscat_support, X_eis, key="r_error")
